...
public class ToggleCommand extends Command {
    ...
    public void buildArguments(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(this.literal("alloff", Text.translatable(this.baseKeyWithParameter("alloff_description"))).executes(this::alloff));
    ...
    public int alloff(CommandContext<CommandSource> context) {
        ...
        InertiaBase.sendChatMessage(Text.translatable(this.baseKeyWithParameter("alloff")));

        return com.mojang.brigadier.Command.SINGLE_SUCCESS;
    }

    public int setState(CommandContext<CommandSource> context, Boolean state) throws CommandSyntaxException {

        Module module = context.getArgument("module", Module.class);
        if (state == null) {//toggle
            module.toggle();
            InertiaBase.sendChatMessage(Text.translatable(this.baseKeyWithParameter("toggled"), module.getId()));
            return com.mojang.brigadier.Command.SINGLE_SUCCESS;
        }
        if (module.isEnabled() == state) {
            InertiaBase.sendChatMessage(Text.translatable(this.baseKeyWithParameter("already_" + (state ? "enabled" : "disabled")), module.getId()));
            return com.mojang.brigadier.Command.SINGLE_SUCCESS;
        }
        module.setState(state);
        InertiaBase.sendChatMessage(Text.translatable(this.baseKeyWithParameter(state ? "enabled" : "disabled"), module.getId()));
